  ...
  module Models
    ...
    class AssignmentScore < AeriesNetApi::Models::BaseModel
      ...
      @@aeries_attributes_list = %w(PermanentID GradebookNumber AssignmentNumber DateCompleted IsMissing PointsPossible
                                    PointsEarned NumberCorrectPossible NumberCorrect Mark PercentCorrect
                                    RuleNumberApplied IsRuleReplacingScore RuleReplacedScore StandardScores)
      ...
      def parse(aeries_data, aeries_attributes_list, setters_list)
        super
        self.date_completed = DateTime.parse(date_completed) unless date_completed.nil?
        standards_list = []
        standard_scores.each do |item|
