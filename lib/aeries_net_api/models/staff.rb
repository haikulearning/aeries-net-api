  ...
  module Models
    ...
    class Staff < AeriesNetApi::Models::BaseModel
      ...
      @@aeries_attributes_list= %w{ID FirstName LastName MiddleName Sex BirthYear BirthDate FullTimePercentage HireDate
        LeaveDate InactiveStatusCode StateEducatorID UserName EmailAddress PrimaryAeriesSchool}
      ...
      def parse(aeries_data, aeries_attributes_list, setters_list)
        super
        self.birth_date=DateTime.parse(self.birth_date) unless self.birth_date.nil?
        self.hire_date=DateTime.parse(self.hire_date) unless self.hire_date.nil?
        self.leave_date=DateTime.parse(self.leave_date) unless self.leave_date.nil?
      end
